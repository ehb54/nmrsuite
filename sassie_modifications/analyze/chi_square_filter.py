...
'''
import os,sys,time,string,locale,random,numpy,time,platform,glob
...
def find_best(variables,txtOutput):
	...
	runname,saspath,sasintfile,io,number_of_weight_files,basis_string,weight_file_names,sastype,reduced_x2,plotflag=unpack_variables(variables)
	...
	x2file='x2file.txt'
        ...
        if sastype != 0:
                ...
                spectra=os.path.join(runname,'chi_square_filter',saspath_base,'spectra')
	...
	x2cl=[] ; x2ch=[] ; rgcl=[] ; rgch=[]
	#runst=runname+'/chi_square_filter/'
...
#        	log.append(saspath+'/'+sasfilestring+mst)
	...
	if (number_of_weight_files > 0):
		...
		chi_helper.write_weight_files(python_basis,x2array,rgarray,weight_file_names,runst)
