...
# For local testing
sys.path.append('./')
import pre_utils as pre_utils

# For complied version
#import sassie.analyze.pre.pre_utils_4plot as pre_utils
...
class pre():
    ...
    def unpack_variables(self, variables):
        ...
        mvars.ratiofile = variables['ratiofile'][0]

        mvars.T2dia = variables['T2dia'][0]
        mvars.Htime = variables['Htime'][0]        
        mvars.freq  = variables['freq'][0]
        mvars.TAUc  = variables['TAUc'][0] 
...
#        mol.read_pdb(mvars.pdbfile)
        ...
        mvars.file_calc_pre = ['' for x in xrange(mvars.number_of_frames + 1)]
    ...
    def run(self, plotQueues):
        '''
        method to perform PRE calculation
        '''
        ...
        for frame in range(0, mvars.number_of_frames):
            pre_utils.pre_core(self,app, frame, plotQueues)
