...
sys.path.append('./')
import altens_utils as altens_utils

#import sassie.analyze.altens.altens_utils as altens_utils
...
class altens():
    ...
    def unpack_variables(self, variables):
        ...
        mvars.residue_list_file_flag = variables['residue_list_file_flag'][0]
...
#        mol.read_pdb(mvars.pdbfile)
        ...
        mvars.file_out = ['' for x in xrange(mvars.number_of_frames + 1)]
        mvars.file_calcrdc = ['' for x in xrange(mvars.number_of_frames + 1)]
#        mvars.file_mc_out = ['' for x in xrange(mvars.number_of_frames + 1)]
        mvars.file_mc_trajectory = ['' for x in xrange(mvars.number_of_frames )]   # Don't need average file
    ...
    def run(self, plotQueues):
        ...
        for frame in range(0, mvars.number_of_frames):
            altens_utils.altens_core(self,app, frame, plotQueues)
