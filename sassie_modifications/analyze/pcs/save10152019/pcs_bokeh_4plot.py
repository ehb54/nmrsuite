...
def read_calc_pcs(self,frame,app):
    
#   Only works for the first chain

    mvars = self.mvars
    calcfile=open(mvars.file_calc_pcs[frame],'r').readlines()
    ...
    for line in calcfile:
        ...
        if ( re.match(r"\d{1,2}", txttrim) ):
            ...
            resnumber.append(int(locale.atof(txt[3])))
            rdc_exp.append(float(locale.atof(txt[4])))
            rdc_calc.append(float(locale.atof(txt[5])))
            delta_rdc.append(float(locale.atof(txt[6])))
