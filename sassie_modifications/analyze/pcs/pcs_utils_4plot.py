...
def pcs_core(self, app, frame, plotQueues):
    ...
    try:
        ...
        if mvars.user_guess_flag:
            vertices = read_vertices(self)
            pgui ("\n<<<< User-guessed initial positions of spin label >>>>\n")
            pgui("="*60+" \n")
            for i in range( vertices.shape[0]):
                message = 'Position ' + str(i+1)  + ' : ( ' +  ', '.join(map(str, [ "{:.3f}".format(x) for x in vertices[i]] )) + ' )'
                pgui( message + "\n")

        else:
            vertices = generate_vertices(self,coord_all, 1) #Generate guesses using all coordinates
            message = "\n<<<< " + str(vertices.shape[0]) + " positions of spin label was autometically guessed >>>>"
            pgui("="*60+" \n")
            pgui( message + "\n")
            for i in range( vertices.shape[0]):
                message = 'Position ' + str(i+1)  + ' : ( ' +  ', '.join(map(str, [ "{:.3f}".format(x) for x in vertices[i]] )) + ' )'
...
#    print vertices
#        vertices = np.array([[35.842,       -95.145,       -25.266]])
    except:
